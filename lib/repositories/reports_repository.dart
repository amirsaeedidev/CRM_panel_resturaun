...
class ReportsRepository {
  ...
  Future<ReportModel> getSalesReport({
    ...
    try {
      // Assumes an RPC function 'get_sales_report' exists in Supabase
  ...
  Future<ReportModel> getTopProductsReport({int limit = 10}) async {
    try {
      // Assumes an RPC function 'get_top_products' exists in Supabase
      final response = await _client.rpc(
        'get_top_products',
  ...
  Future<ReportModel> getCustomerDistributionReport() async {
    try {
      // Assumes an RPC function 'get_customer_distribution' exists
